...
const Navbar = ({ onToggleSidebar }) => (
    <nav style={styles.navbar}>
        ...
        <div style={styles.navRight}>
            <button onClick={onToggleSidebar} aria-label="Toggle sidebar" style={{ ...styles.btn, padding: '8px 10px', borderRadius: '8px' }}>
...
const ActionSidebar = ({ visible = true }) => (
    <div style={{ ...styles.actionSidebar, transform: visible ? 'translateX(0)' : 'translateX(120%)', transition: 'transform 280ms ease' }} aria-hidden={!visible}>
        <div style={styles.profileFollowGroup}>
            <div style={styles.profilePicContainer}>
                <img src="/hem.png" alt="Profile" style={styles.sidebarProfilePic} />
        ...
        <div style={styles.mainContainer}>
            <div style={styles.content}>
                {images.map((img, index) => (
                    <img key={index} src={img} alt={`Project ${index+1}`} style={styles.contentImg} />
                ))}
            </div>
            {/* ActionSidebar will be controlled by App via prop */}
            <ActionSidebar />
        </div>
    )
}
...
const ProjectHeader = ({ likes, views, comments, date }) => {
    ...
    return (
        <header style={styles.topHeader}>
            ...
            <p style={styles.projectStatsHeader}><i className="fas fa-eye"></i> {views} &bull; <i className="fas fa-comment"></i> {comments}</p>
                ...
                <div>
                    <div style={{ ...styles.ownerSection, ...styles.card }}>
                        ...
                        <div style={styles.profileInfo}>
                            <img src={project.owner.pic} alt={project.owner.name} style={styles.ownerProfilePic} />
                            <div style={styles.profileDetails}>
                                <p style={styles.profileName}>{project.owner.name}</p>
                                <p style={styles.profileLocation}>📍 {project.owner.location}</p>
                            </div>
                        </div>
                        <button style={{ ...styles.btnDetail, ...styles.btnFollowDetail }}><i className="fas fa-plus"></i> Follow</button>
                    ...
                    <div style={{ ...styles.projectInfoSection, ...styles.card }}>
                        ...
                        <p style={styles.projectStats}><i className="fas fa-thumbs-up"></i> {project.stats.likes} &bull; <i className="fas fa-eye"></i> {project.stats.views} &bull; <i className="fas fa-comment"></i> {project.stats.comments}</p>
...
const App = () => {
    const projectData = {
        ...
        date: "July 30th 2025",
        stats: { likes: 1, views: 43, comments: 0 },
        owner: { name: "Hemanth", location: "Tamilnadu, India", pic: '/hem.png' },
    ...
    return (
        <div style={styles.body}>
            ...
            {/* Pass sidebar visibility to ProjectGallery so the ActionSidebar can be synced if needed */}
            <div style={{ position: 'relative' }}>
                <ProjectGallery />
                {/* floating control for small screens: show a small handle to toggle */}
            </div>
            <ProjectHeader likes={projectData.stats.likes} views={projectData.stats.views} comments={projectData.stats.comments} date={projectData.date} />
            <AuthorSection authorName={projectData.owner.name} authorPic={projectData.owner.pic} />
            <DetailsSidebar project={projectData} />
            {/* Render the sidebar at root level so it's always positioned correctly and controlled by state */}
            <ActionSidebar visible={isSidebarOpen} />
            <Footer />
        </div>
    )
}
